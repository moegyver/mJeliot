 ...
 * @author nmyller
 */
public class ClassActor extends Actor implements ActorContainer {
    ...
    /**
     * How many variables actually are on the ClassActor at the moment.
     */
     ...
     */
    public ClassActor(String name, int varCount) {
        this.name = name;
        this.varCount = varCount;
        insets = new Insets(2, 6, 4, 6);

        setDescription("Allocated space for static variables from class "
                + name);
    }
    ...
    public Animation disappear() {
        return new Animation() {
            ...
            public void animate(double pulse) {
                ...
                Tracker.trackTheater(TrackerClock.currentTimeMillis(),
                        Tracker.MODIFY, getActorId(), Tracker.RECTANGLE,
                        new int[] { p.x }, new int[] { p.y }, ClassActor.this
                                .getWidth(), ClassActor.this.getHeight(), 0,
                        -1, getDescription());
                h += plus * pulse;
                size.height = (int) h;
                setSize(size);
